"""Tool for verifying annotations using GPT-4 vision API."""
import base64
import os
import requests
...
class CheckAnnotationColor():
    """Tool for verifying annotation correctness using GPT-4 vision API."""
    
    def __init__(self, image_dir: str, info_dir: str, prompt_dir: str, 
        ...
        Args:
            ...
            annotation_dir: Directory containing annotations to verify
...
if __name__ == "__main__":
    ...
    prompt_dir = os.path.join(prompt_root, 'check_annotation_example')
    info_dir = os.path.join(data_root, 'annotations/color')
    save_dir = os.path.join(output_root, 'annotation_check')
    all_image_dir = os.path.join(data_root, 'train/images')
    caption_dir = os.path.join(output_root, 'captions/10_1')
    annotation_dir = os.path.join(output_root, 'annotations/v3')
    ...
    #     get_response(image_name)

    # # image_names = ["0000167_02184_d_0000135.jpg"]
    checkcolor = CheckAnnotationColor(prompt_dir=prompt_dir, info_dir=info_dir, image_dir=image_dir, save_dir=save_dir, all_image_dir=all_image_dir, caption_dir=caption_dir, annotation_dir=annotation_dir, n=1)
    for image_name in image_names:
    ...
    # res_content = get_response(image_path)
    # print(res_content)