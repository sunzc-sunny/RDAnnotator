...
image_annotation_check = "/home/sunzc/VisDroneAnnotation/night_noncolor_check_annotation"
image_reannotation_annotation = "/home/sunzc/VisDroneAnnotation/night_noncolor_regenerate_annotation"

image_path = "/data/sunzc/VisDrone2019/VisDrone2019-DET-train/images"

ann_path = "/home/sunzc/VisDrone2019/visdrone_anchor_normalize_ann_new"

save_path = "/home/sunzc/VisDrone2019/non_ann_train_noncolor_vis"


image_path = "/data/sunzc/VisDrone2019/all_image"

ann_path = '/home/sunzc/VisDroneAnnotation/test_checked/noncolor_normalize'

save_path = '/home/sunzc/VisDroneAnnotation/test_noncolor_rerecheck'
...
def vis_image(ann_image_name):
    ...
    for ann_info in ann_infos:
        ...
        for match in matches:
            ...
            cv2.putText(image_vis1, str(x), (int(x*image.shape[1]), int(y*image.shape[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 1)
            class_line = [x, y]
        ...
        for i in range(len(bbox_infos)):
            ...
            if class_name in bbox_info:
                matches = re.findall(pattern, bbox_info)
                cv2.circle(image_vis2, (int(float(matches[0][0])*image.shape[1]), int(float(matches[0][1])*image.shape[0])), 2, (0, 255, 0), 4)
                x = bbox_info.split('[')[1].split(',')[0]
                cv2.putText(image_vis2, str(x),(int(float(matches[0][0])*image.shape[1]), int(float(matches[0][1])*image.shape[0])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        ...
        for ann_info, check_info in zip(ann_infos, check_infos):
            ...
            for i in range(len(bbox_infos)):
                ...
                if class_name in bbox_info:
                    ...
                    # class_list.append([float(matches[0][0]), float(matches[0][1])]) 
            image_show = cv2.hconcat([image_vis1, image_vis2])
